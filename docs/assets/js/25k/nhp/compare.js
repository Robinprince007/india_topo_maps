...
const GRID_LAYER_INDEX = 'SOI NHP Index Grid';
...
const soi_attribution = makeLink('https://onlinemaps.surveyofindia.gov.in/FreeMapSpecification.aspx', '1:25000 NHP Maps') +
                    ' &copy; ' +
...
function getSOILayer() {
    const src = new ol.source.XYZ({
        url: 'https://indianopenmaps.fly.dev/soi/25k/nhp/{z}/{x}/{y}.webp',
...
function getGridSource() {
    const src = new ol.source.Vector({
        ...
        url: '/india_topo_maps/25k/nhp/nhp_index.geojson',
        overlaps: false,
        attributions: [
            makeLink("https://onlinemaps.surveyofindia.gov.in/FreeOtherMaps.aspx", "SOI NHP Index(simplified)")
        ]
    });
    return src;
}
...
function getIndiaOutlineSource(map1, map2, container, statusFn) {
    const url = '/india_topo_maps/25k/nhp/india_boundary.geojson';
    const src = new ol.source.Vector({
        format: new ol.format.GeoJSON(),
        url: url,
        overlaps: false,
    ...
    function addPopup(layer, map) {
      ...
      map.on('click', function(e) {
        ...
        showPopup(map, e, popup, (f) => {
            const sheetNoRaw = f.get('id');  // Changed from 'EVEREST_SH' to 'id'
            const sheetNo = sheetNoRaw.replace(/_/g, '/');  // Convert underscores to slashes
